...
The non-`--live` `verify` runs in CI on every PR and on push to `main` (`.github/workflows/verify.yml`), so the ten deterministic checks gate merges automatically.
