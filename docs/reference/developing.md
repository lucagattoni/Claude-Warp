...
The non-`--live` `verify` runs in CI on every PR and on push to `main` (`.github/workflows/verify.yml`), so the thirteen deterministic checks gate merges automatically, alongside `scripts/gate-selftest.sh` which gates the gate itself.
