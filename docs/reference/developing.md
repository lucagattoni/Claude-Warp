...
| `scripts/dev.sh portability [--docker]` | Fills the runner templates and **executes** them against stub `claude` binaries, asserting observed exit codes: a missing binary → 127, an unresolved slash command → 4, budget exhaustion → 6 after exactly one attempt, `CLAUDE_BIN` outranking a native install, the fan-out counting *and* launching an unterminated last task, and the guard reopening the day after a `handoff`. The wall-clock cap must be either enforced or **loudly declared unenforced** — never silently absent. `--docker` runs the same file inside debian (bash 5.x, real `timeout`) both with and without `python3`, so the environment-dependent branches are exercised in the environments that differ. No tokens, no auth. |
| `scripts/dev.sh verify` | Fourteen deterministic checks (no LLM, no tokens): source integrity, the setup-is-dynamic regression guard, the install copy contract, setup-template placeholder fill, docs coherence (every skill has a `### `/<skill>`` section in `docs/reference/skills.md` + a README row), the shared-executable self-tests (`verifier-lib.sh` + `ledger.sh` + `reviewer-guard.sh` each run their own `--self-test`), behavioural-claim count coherence (the `M/N` verified-live count is computed from the registry and asserted identical in `BEHAVIOURAL-CLAIMS.md` and `docs/reference/architecture.md`), and plugin-manifest version coherence (`.claude-plugin/plugin.json` version equals `VERSION` — the read-only release gate never edits it, so this pins it). Checks 10 and 11 **execute** filled runners rather than grepping them — including `--worktree` mode against a real bare origin (clean run leaks no worktree or branch; a push that landed before a budget cap is reported as a durable trace; one that pushed nothing is not). Exits non-zero on failure, naming every failing check — suitable for CI. |
| `tests/dogfood/retro-multirun/build.sh` | Builds a 12-run loop fixture (mixed verdicts, real dated git history, a sibling loop, and a gitignored guard-skip log) so `/claude-warp-retro` can be exercised against N>1 history. Pure shell + git, zero tokens. `ANSWER_KEY.md` is the oracle; `OBSERVED-RETRO-*.md` is the first passing output. |
| `scripts/gate-selftest.sh` | Proves `verify` can still **report** a failure: mirrors the working tree into a sandbox, plants known failures (a bad skill name, a drifted claim count, `claude` off `PATH`) and asserts the gate reaches all 14 checks, prints `VERIFY FAILED`, and counts the issues correctly. A gate that cannot fail is not a gate — and only mutating the artifact under it reveals that. Runs in CI beside `verify`. |
...
The non-`--live` `verify` runs in CI on every PR and on push to `main` (`.github/workflows/verify.yml`), so the fourteen deterministic checks gate merges automatically, alongside `scripts/gate-selftest.sh` which gates the gate itself.
...
| [**claude-code-harness** — *CCH TeamAgent Debate*](https://github.com/Chachamaru127/claude-code-harness) | Chachamaru127 | The AI-residuals epistemic-honesty scan (`scripts/check-ai-residuals.sh`); reconcile-and-re-ticket closure (`claude-warp-converge`); the severity→verdict gating honesty rider (v0.28.0); the red-team / Skeptic "try-to-break" reviewer charter + trivially-passing-AC check (v0.29.0); the named plan-validation modes + 5-lens plan validation (`validation.mode` + spec-alignment / memory-reuse / product-fit / security-fit / works-in-practice, contract Phase 6, v0.37.0); the OpenSSF Scorecard supply-chain posture + `SECURITY.md` (`scorecard.yml`, v0.37.0); the machine-readable review verdict (`review-result.v1`) + the enforcing `review-gate` hook that gates the `Stop` event on `APPROVE` with 0 open critical/major (`new-hook`, v0.38.0) |
